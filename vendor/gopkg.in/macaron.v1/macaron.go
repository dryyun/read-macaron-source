...
// handlerFuncInvoker 是对 func(http.ResponseWriter, *http.Request) 的快速注入包装
...
func validateAndWrapHandler(h Handler) Handler {
	if reflect.TypeOf(h).Kind() != reflect.Func { // 反射判断一定要是 func 类型
	...
	if !inject.IsFastInvoker(h) {
		...
		case func(http.ResponseWriter, *http.Request):
			return handlerFuncInvoker(v) // handlerFuncInvoker 是定义的 type，这里是 [类型转换]
